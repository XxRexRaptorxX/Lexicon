...
- A __power component__ provides power to other parts of a circuit [e.g. *Redstone Torch*] 

- A __transmission component__ passes power from one part of the circuit to another [e.g. *Redstone Dust*] 

-  A __mechanism component__ affects the environment (by moving, light, etc.) [e.g. *Piston*]
...
*Redstone Torches* can be used to power blocks and transmission components such as *Redstone Dust*, activate mechanism components such as [*Pistons*](./redstone_components.md#piston), or invert redstone 
signals 
like a 
[*NOT Gate*](./redstone_circuits.md).
...
A *Redstone Block* acts as a permanently powered redstone power source that can be pushed by [*Pistons*](./redstone_components.md#piston).
...
A [*Comparator*](./redstone_components.md#comparator) can read the state, and it emits a signal strength of 15 if the bulb is lit. 
If the bulb is unlit, the [*Comparator*](./redstone_components.md#comparator) does not emit any power, allowing this [*Comparator*](./redstone_components.md#comparator) and *Copper Bulb* combination to act as a 
compact [T Flip-Flop](./redstone_circuits.md#t-flip-flop).
...
Because disabled slots prevent items from entering the slot, *Crafters* can be used to craft any item automatically without any input from the [*Player*](../creatures/human-player.md), using a series of [*Hoppers*](./redstone_components.md#hopper) and/or [*Droppers*](./redstone_components.md#dropper) & the correct configuration of disabled slots for the recipe.
...
If the *Minecart* on the *Detector Rail* is a *Minecart* with Chest or *Minecart with Hopper*, an adjacent [*Comparator*](./redstone_components.md#comparator) facing away from the *Detector Rail* outputs a power level proportional to the container's fullness.
...
Each vibration in the game falls under a certain [amplitude value](https://minecraft.fandom.com/wiki/Sculk_Sensor#Vibration_Resonance). This value can be measured with a [*Comparator*](./redstone_components.md#comparator).
